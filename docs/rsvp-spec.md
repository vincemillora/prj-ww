...
| `app/(protected)/layout.tsx` | Sidebar-less shell — centered `max-w-[1300px]` container on **white** (page-corner floral sprays sit in their own `absolute inset-0 overflow-hidden` layer so their bleed clips without cutting page chrome; horizontal scroll guarded on `<body>`); `admin-surface` themes selection/caret/scrollbar from the ink; top-right `AccountMenu`. |
