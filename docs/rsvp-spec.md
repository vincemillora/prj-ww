...
| `app/(protected)/dashboard/guests-board.tsx` (+ `board/` subcomponents and `board/board-data.ts`) | Client kanban board (imported design): 3 drag-and-drop status columns on `md+` (HTML5 dnd, `useOptimistic` status flip → `moveGuestStatus`), status tab pills + card list on phones; search + label-filter dropdown; per-column “Show more” (20 at a time). Filtering, grouping, and progress calculations live in the tested pure data module. Column tints/inks are **CSS variables** (`--{pending,going,declined}-{wash,wash-over,edge,mark,ink}` in `app/globals.css`), not hardcoded hexes — a lane is retuned in one place and the dark variant follows for free. |
