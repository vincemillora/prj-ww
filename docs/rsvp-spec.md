...
| `app/login/page.tsx` | "Continue with Google" + pending/error messaging; redirects already-signed-in active admins to `/dashboard`. Stands on the guest **drapery** (`LaceBackdrop`, shared with `/`), carries the `lace-page` class so the document canvas paints in ink behind Safari's chrome, and centres one white card holding the couple's monogram at 96px. The access-restriction line lives **inside** the card: white 12px type over the drapery is exactly the case the hero's measured scrim table says not to trust. |
| `app/(protected)/layout.tsx` | Sidebar-less shell — centered `max-w-[1300px]` container on **white**; `admin-surface` themes selection/caret/scrollbar from the ink; top-right `AccountMenu`. **One** page-corner spray (bottom-right, `sm`+), in its own `absolute inset-0 overflow-hidden` layer so the bleed clips without cutting page chrome; horizontal scroll guarded on `<body>`. The imported design's top-left spray and its phone top-right mirror were dropped 2026-09-02: that corner is the masthead, and at any usable opacity the blooms crossed the letterforms of the couple's name and of “Manage RSVP”. The offsets are 16/20px rather than the imported 46/52 because a bloom cut in half by the clip edge reads as broken art — at these offsets only the stem's tail crosses. |
| `app/(protected)/dashboard/page.tsx` | Single-page **“Manage RSVP”** — masthead (script nameplate + sprig, title, date line, countdown), account chip, and the board. Export CSV and Add guest sit in the header on `sm+` only; on phones Add guest is the round button in the board toolbar. |
| `app/(protected)/dashboard/guests-board.tsx` (+ `board/` subcomponents and `board/board-data.ts`) | Client kanban board (imported design): 3 drag-and-drop status columns on `md+` (HTML5 dnd, `useOptimistic` status flip → `moveGuestStatus`), status tab pills + card list on phones; search + label-filter dropdown; per-column “Show more” (20 at a time). **Toolbar diverges by breakpoint (2026-09-02):** desktop keeps the open search field, and Add guest stays in the page header beside Export CSV; phones get the search field behind a round 44px button (`board/search-popover.tsx`, a **Popover** and not a DropdownMenu — a menu owns the keyboard for typeahead and would fight a text field) next to a round 44px Add guest (`GuestDialog compact`), with the Tags pill grown to 44px to match. The search trigger fills in while a query is active, since once the field closes it is the only thing left that can say the board is filtered. This replaced a **fixed bottom Add guest bar** that covered the last card and followed the admin down every scroll. Filtering, grouping, and progress calculations live in the tested pure data module. Column tints/inks are **CSS variables** (`--{pending,going,declined}-{wash,wash-over,edge,mark,ink}` in `app/globals.css`), not hardcoded hexes — a lane is retuned in one place and the dark variant follows for free. |
...
| `components/dashboard/florals.tsx` + `florals/{stem-geometry.ts,stem-geometry.test.ts,spray.tsx}` | Decorative botanicals (server components), split the way the letter splits `our-story/vine-geometry.ts`: `stem-geometry.ts` is the maths that hangs a plant on a stem (segment types, `toPath`, `at`, `orient`, the plants' measured stem points and bearings) and is **unit-tested**, `spray.tsx` renders it, and `florals.tsx` is composition only — which run goes where and which plant sits at which point. Exports `PageFloralBottomRight`, `NameSprig`, `AccountGarland`, `ColumnVineBottomLeft/TopRight`. **Restyled 2026-09-02:** the imported design's stem geometry is kept verbatim (still edge-anchored per the −10%-of-height rule above), but every leaf and bloom is now the guest letter's own asset — `/florals/{rose-bloom,leaf-large,leaf-small}.svg`, the three plants Our Story grows — drawn as CSS-masked silhouettes that take their colour from the call site (`bg-current`), so a status lane's vine wears that lane's pigment while a page corner is a quiet ink watermark. The hand-drawn `Blossom`/`Leaf` primitives, the `CoupleFigures` cartoon couple, and the 81 hardcoded wisteria/rose hexes are gone. Each sprig is two spans: the outer places it, the inner carries `wind-rustle` — that animation sets `transform`, which would otherwise replace the placement transform outright. `aria-hidden`, `pointer-events-none`. |
| `lib/guest-token.ts` | Short unguessable invite-token generator (crypto). |
| `app/(protected)/dashboard/users/page.tsx` | User management (superadmin only). **Responsive as of 2026-09-02:** phones get stacked, labelled records with a full-width action button; the table returns at `sm`. A five-column admin table cannot fit 390px, and `overflow-x-auto` was not an answer here — role and status truncated mid-word and the Activate/Deactivate control, the only reason to open the page, sat off-screen behind a scroll with no affordance. Both branches render the same rows from the same source. |
